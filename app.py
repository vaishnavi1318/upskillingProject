...
# Route to render job details page by job_id
@app.route('/recruiter/job/<int:job_id>')
def recruiter_job_page(job_id):
    return render_template('recruiter_job_details.html', job_id=job_id)

# Route to fetch all jobs as a JSON response
...
# Route to render the student login page
...
def apply_job_route():
    data = request.get_json()
    usn1 = session.get('usn')  # Get the 'usn' from the session
    job_id = data.get('job_id')  # Get 'job_id' from the request data
    
    # Check if 'usn' and 'job_id' are valid
    if usn1 and job_id:
        apply_job(usn1, job_id)  # Call apply_job function (assuming this function is defined elsewhere)
        return jsonify({'success': True})
    else:
...
# Route to handle GET requests for jobs by status
@app.route('/api/jobs_by_status', methods=['GET'])
def api_get_jobs_by_status():
    usn2 = session.get('usn')  # Get 'usn' from session
    
    # If the 'usn' is not found in the session, return an error
    if not usn2:
        return jsonify({'error': 'Unauthorized'}), 401
    
    # Assuming get_jobs_by_application_status is defined elsewhere
    categorized_jobs = get_jobs_by_application_status(usn2)  
    return jsonify(categorized_jobs)

# Route to handle GET request for logout
@app.route('/logout')
def logout():
    session.clear()  # Clear the session
    return redirect(url_for('student_login'))  # Redirect to login page

# Route to handle GET requests for job details by job_id
@app.route('/api/job_details/<int:job_id>', methods=['GET'])
def job_details(job_id):
    # Assuming fetch_job_details is defined elsewhere
    job = fetch_job_details(job_id)  # Fetch job details by job_id
...
def update_student_password(usn, email, new_password):
    # Placeholder for password update logic (e.g., update in database)
