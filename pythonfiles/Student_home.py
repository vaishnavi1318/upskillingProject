...
def get_jobs_by_application_status(usn):
    ...
    cursor1.execute('''SELECT jobs.*, CASE WHEN student_applications.job_id IS NOT NULL THEN 'applied' ELSE 'not applied' END AS status 
                       FROM jobs 
                       LEFT JOIN student_applications ON jobs.id = student_applications.job_id AND student_applications.usn = ?''', (usn,))
